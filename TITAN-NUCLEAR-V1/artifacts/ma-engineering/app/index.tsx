...
export default function SplashScreen() {
  ...
  // Use state-driven <Redirect> instead of router.replace() — much more
  // reliable in Expo Router because it's declarative and not timing-sensitive.
  ...
  useEffect(() => {
    ...
    logoScale.value   = withSpring(1, { damping: 9, stiffness: 90 }, () => {
      idleSpin.value = withRepeat(withTiming(360, { duration: 5000, easing: Easing.linear }), -1, false);
    ...
    glowPulse.value = withDelay(900, withRepeat(
      withSequence(
    ...
    const timer = setTimeout(() => {
      diagLog("Splash timer fired — setting ready=true for <Redirect>");
      setReady(true);
    }, 3400);
  ...
  // Declarative navigation — renders <Redirect> when timer fires.
  // This is the safest way to navigate in Expo Router (no timing issues).
  if (ready) {
    diagLog("Rendering <Redirect href='/(tabs)' />");
  ...
  const shimmerStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: shimmerX.value * 140 }, { rotate: "20deg" }],
  }));
      ...
      <View style={styles.center}>
        ...
        <View style={styles.titleRow}>
          {letters.map((ch, i) => (
            <ReAnimated.Text
              key={`${ch}-${i}`}
              entering={FadeInDown.duration(450).delay(750 + i * 55).springify().damping(11)}
              style={styles.title}
            >
              {ch === " " ? "\u00A0" : ch}
            </ReAnimated.Text>
        ...
        <ReAnimated.Text entering={FadeIn.duration(700).delay(1250)} style={[styles.welcome, welcomeGlowStyle]}>
          Welcome
        </ReAnimated.Text>
        <ReAnimated.Text entering={FadeIn.duration(600).delay(1500)} style={styles.subtitle}>
          Powered by TITAN AI
        </ReAnimated.Text>
...
const styles = StyleSheet.create({
  ...
  logoRing: {
    ...
    elevation: 20, overflow: "hidden", backgroundColor: "#0D0D2B",
    borderWidth: 2, borderColor: "#00B4FF40",
  },
  logoImage:     { width: "100%", height: "100%" },
  shimmerStreak: { position: "absolute", top: -40, bottom: -40, left: -10, width: 26, backgroundColor: "rgba(255,255,255,0.35)" },
  titleRow:      { flexDirection: "row", marginBottom: 6 },
  title:         { fontSize: 30, fontFamily: "Inter_700Bold", color: "#00B4FF", letterSpacing: 4 },
  welcome: {
    fontSize: 22, fontFamily: "Inter_700Bold", color: "#00FFD1",
    letterSpacing: 3, marginTop: 4, marginBottom: 14,
    textShadowColor: "#00FFD1", textShadowOffset: { width: 0, height: 0 },
  },
  subtitle:    { fontSize: 12, fontFamily: "Inter_400Regular", color: "#8899AA", letterSpacing: 3, marginBottom: 32 },
  progressBg:  { width: width * 0.5, height: 2, backgroundColor: "#0D0D2B", borderRadius: 2, overflow: "hidden", marginBottom: 24 },
  progressFill:{ height: "100%", backgroundColor: "#00B4FF", borderRadius: 2 },
  brand:       { fontSize: 10, fontFamily: "Inter_600SemiBold", color: "#00B4FF60", letterSpacing: 4 },
});
